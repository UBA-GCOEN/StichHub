...
const Mainpage = () => {
  return (
    <div className="bg-primary justify-center text-center h-screen">
      ...
      <div className="text-3xl mt-14 font-bold text-center justify-center text-white">or</div>
      <div className="text-3xl mt-3 font-bold text-center justify-center text-white">Create Your New Measurement</div>
      <div>
...
const SizeCalculator = () => {
  ...
  return (
    <div className="bg-primary h-fit flex flex-col  text-white">
      {/* {isShown && ( */}
      <h1 className="text-center text-3xl mx-10 mt-10 font-bold">Create Your Measurement</h1>
      <div className="text-left ml-14 lg:ml-[18%] mt-10">
      <label className="mt-10 text-xl ">
        <label className="text-left">Height (cm)</label><br></br>
        <input
          type="range"
          value={minValue}
          min={130}
      ...
      <label className=" text-xl ml-14 lg:ml-[18%] mt-3">
        <label className="text-xl text-start">Gender</label><br/>
        <div className="mt-3" >
        <input type="radio"  checked={gender=="male"} name="gender" className="mx-2" value="male" onChange={handleOptionChange}/>
        <label >Male</label>
        <input type="radio" checked={gender=="female"} name="gender" className="mx-2 ml-10" value="female" onChange={handleOptionChange}/>
        <label >Female</label>  
        </div> 
      ...
      <button className="bg-blue-500 w-fit ml-[35%] lg:ml-[47%]  px-5 mt-5 text-xl font-semibold py-3 mx-20 rounded-2xl drop-shadow-md shadow-inner-xl" onClick={()=>{handleClick(); handleCalculate();}} >Calculate</button>
      {/* )} */}
      {isShown && (
        <div className="ml-[10%] mr-[10%]  mt-[10%] text-center">
          <div className="text-white text-3xl font-bold mb-3 ">Give Your Profile a Name</div>
          <input type="text" Placeholder="Profile1" className="bg-gray-700 text-gray-300 font-semibold text-center rounded-sm"/>
          <br/><button className="bg-yellow-400 text-black font-bold mt-3 w-[15%] lg:w-[4%] rounded-md items-self-center"><img className="" src="https://images.squarespace-cdn.com/content/v1/5c38d342e2ccd16874962329/1555870468349-36N6I1DQ4711MNPG0ZQ0/blog+bw+trans.png"/></button>
          <h3 className="text-xl font-bold text-gray-300 mt-4">Given Below are the measurements generated by our system.</h3>
          <div className="grid grid-col-3 mb-3 gap-2 text-black mt-10 ">
          <div className="lg:py-2 bg-white text-center rounded- px-1"><p className="text-lg font-semibold">Neck Size</p> {neckSize} cm</div>
          <div className="lg:py-2 col-start-2 bg-white rounded-sm justify-center text-center col-end-3 px-1"><p className="text-lg font-semibold">Chest Size</p> {chestSize} cm</div>
          <div className="col-start-3 lg:py-2 bg-white rounded-sm justift-center text-center col-end-4 px-1"><p className="text-lg font-semibold">Shoulder Size</p> {shoulderSize} cm</div>
          <div className="bg-white lg:py-2 text-center rounded-sm px-1"><p className="text-lg font-semibold">Waist Size</p> {waistSize} cm</div>
          <div className="bg-white lg:py-2 text-center rounded-sm px-1"><p className="text-lg font-semibold">Arm Length</p> {armLength} cm</div>
          <div className="bg-white lg:py-2 text-center rounded-sm px-1"><p className="text-lg font-semibold">Inseam</p> {inseam} cm</div>
          </div>
          <Link to="/Mainpage" className="underline text-gray-400 hover:text-gray-200">+6 More measurements</Link><br/>

          <button className="px-3 mt-10 py-2 text-center text-white bg-gray-700 rounded-xl">Go to 3D Model</button>
        </div>
      )}
    </div>
  );
