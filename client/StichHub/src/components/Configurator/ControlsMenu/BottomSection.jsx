...
const BottomSection = () => {
  const { shirtPart, part, setPart, nextPart, prevPart, sleeveType } = useCCustomization();
      ...
      <div className="">
        <div id="parts" className={`flex justify-between cursor-pointer`}>
          ...
          shirtPart.map((item, index) => (
            <div
              key={index}
              className={`border-b-4 ${(index < part) ? 'border-indigo-500' : 'border-gray-200'} p-1 mt-2 text-sm`}
              onClick={() => setPart(index)}
            >
              {item.text}
            </div>
          )) :
          shirtPart.slice(0, 3).map((item, index) => (
            <div
              key={index}
              className={`border-b-4 ${(index < part) ? 'border-indigo-500' : 'border-gray-200'} p-1 mt-2 text-sm`}
              onClick={() => setPart(index)}
