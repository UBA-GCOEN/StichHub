  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        "https://stichhub-backend.vercel.app/test/send-email",
        {
          method: "POST",
          headers: {
      ...
      <div className="relative z-[5] flex items-center justify-center">
        ...
        <div id="leftside" className="lg:block hidden mb-[20px] bottom-[20p]">
          <img
            src={ContactUsVector}
            className=""
            alt="a cartoon character pointing at a large tablet with icons"
            loading="lazy"
          />
        </div>
          ...
          <form id="form-Container" className="">
            ...
            <div id="Phone no." className=" py-2 ">
              ...
              <PhoneInput
                ...
                className="mt-[10px] h-10 block width-auto pl-[45px] py-[16px] bg-transparent border-2 rounded-md text-xl shadow-sm placeholder-slate-400 text-white focus:font-medium
                focus:outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500
                disabled:bg-slate-50 disabled:text-slate-500 disabled:border-slate-200 disabled:shadow-none
                invalid:border-pink-500 invalid:text-pink-600
                focus:invalid:border-pink-500 focus:invalid:ring-pink-500"
