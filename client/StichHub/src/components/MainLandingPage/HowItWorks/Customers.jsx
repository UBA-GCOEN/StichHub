  ...
  return (
    <div className="h-[100%]  lg:h-[100vh] relative pt-0  lg:px-20 lg:mx-0 mx-7 lg:mt-0  3xl:mt-0">
      <div className="relative z-[5] ">
        {/* Title */}
        <div id="title" style={{paddingLeft:"8.2rem"}} className="relative flex flex-col">
        ...
        {/* Steps */}
        <div id="steps" className="hidden lg:flex justify-space-between gap-x-10 mx-auto justify-center">
          {/* Steps */}
          {hiwcustomer.map((item, index) => (
            <div
              key={index}
              id="step1"
              className="w-[250px] flex flex-col  items-center mt-16 group cursor-pointer"
            >
              <div
                id="stepNumber"
                className={`relative group-hover:scale-110 after:group-hover:scale-100 group-hover:transition-transform group-hover:duration-300 py-[10px] bg-[#B100AA] text-center w-[60px] h-[60px] rounded-full  ${item.step!=4 ? "after:content-[''] after:w-[240px] after:h-[1px] after:bg-white after:absolute after:top-[50%] after:left-[100%] lg:after:w-[200px] xl:after:w-[210px] 2xl:after:w-[250px] 3xl:after:w-[440px]" : null}`}
              >
                {item.step}
              </div>

              <img id="stepLogo" src={item.img} className="mx-[80px] my-[50px] lg:w-[80px] group-hover:scale-110 group-hover:transition-transform group-hover:duration-300" alt={item.title} loading="lazy" />

              

              <div id="stepTitle" className={`text-xl group-hover:scale-110  group-hover:transition-transform group-hover:duration-300 ${item.style}`}>{item.title}</div>
            </div>
          ))}
        </div>
